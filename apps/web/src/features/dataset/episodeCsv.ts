...
const normalizeHeader = (header: string) => header.replace(".pos", "").trim();
...
export const parseEpisodeCsv = (rawText: string, options: EpisodeCsvParseOptions = {}): EpisodeCsvParseResult => {
  ...
  const timestampIndex = headers.findIndex((header) => header.toLowerCase() === TIMESTAMP_HEADER);

  if (timestampIndex === -1) {
    return { error: "Data file must have a 'timestamp' column" };
  }

  const allowedJointSet = options.allowedJoints !== undefined ? new Set(Array.from(options.allowedJoints)) : null;
