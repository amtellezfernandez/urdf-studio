 ...
 */
export const applyMapping = (jointPositions: Record<string, number>, mappings: JointMapping[], degToRad: boolean): Record<string, number> => {
...
export const validateMapping = (
  ...
  const unmappedDatasetJoints = mappings.filter((m) => !m.urdfJoint || m.urdfJoint === "").map((m) => m.datasetJoint);
