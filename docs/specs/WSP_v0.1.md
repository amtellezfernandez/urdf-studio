...
Blender-imported complete mesh additions currently enter Studio as cube world objects with `simulation.semantic_role = "blender_import"` and the exported Blender material or object color when available. Existing object transforms can be applied automatically; camera edits and deletions still require review before they are applied to a package.
