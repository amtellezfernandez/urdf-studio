 ...
 * - Camera frame: X=forward viewing direction, Y=left, Z=up
 *
...
const CAMERA_POSE_COMPUTE_CONFIG = CAMERA_POSE_COMPUTE_PARAMS;
const DEFAULT_MARGIN_FORWARD = CAMERA_POSE_COMPUTE_CONFIG.defaultMarginForwardMeters;
...
function autoComputeCameraPose(
  ...
  if (options.targetPosition) {
    ...
    if (aimDirection.length() > CAMERA_POSE_COMPUTE_CONFIG.directionLengthEpsilon) {
  ...
  const upAxis = Math.abs(linkUp.dot(aimDirection)) > CAMERA_POSE_COMPUTE_CONFIG.upAxisParallelDot ? worldUp : linkUp;
  ...
  const upRef = Math.abs(xAxis.dot(upAxis)) > CAMERA_POSE_COMPUTE_CONFIG.upAxisParallelDot ? worldUp : upAxis;
