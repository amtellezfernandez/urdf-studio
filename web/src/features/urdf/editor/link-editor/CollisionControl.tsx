...
import type { CollisionData, LinkData } from "@/shared/lib/urdfBrowser";
...
type GeometryNumberFieldRowProps = {
  label: string;
  paramKey: string;
...
const GeometryNumberFieldRow = ({
  label,
  paramKey,
  params,
  onParamChange,
}: GeometryNumberFieldRowProps) => {
  return (
    <BlenderPropertyRow label={label}>
      <NumberInput
        value={parseFloat(params[paramKey] || "1")}
        onValueChange={(value) => onParamChange(paramKey, String(value))}
        step={0.01}
        min={0.001}
        compact
        className="w-20"
...
export const CollisionControl = ({
  ...
  const handleParamChange = createGeometryParamChangeHandler(
    ...
    scheduleUpdate,
    {
      ...
      onBeforeSchedule: () => setCalculationInfo(null),
    }
  );
  ...
  return (
    ...
    >
      <div className="space-y-0.5">
        ...
        {onUrdfChange && (
          <LinkEditorRemoveButton label="Remove" onRemove={onRemove} />
        )}
      </div>
    </BlenderPanel>
  );
