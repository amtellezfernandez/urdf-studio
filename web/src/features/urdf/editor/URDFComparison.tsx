...
import {
  ...
  normalizeAxes,
  prettifyUrdf,
} from "./urdfEditorActions";
import { repairMeshPathsWithIlu } from "@/shared/lib/iluStudio";
import {
...
export const URDFComparison = ({
  ...
  const handleSave = () => {
    try {
      const parsed = parseURDF(editedVizUrdf);
      if (!parsed.isValid) {
  ...
  const handleFixMeshPaths = () => {
    const currentContent = isEditing ? editedVizUrdf : vizUrdf;
    const result = repairMeshPathsWithIlu(currentContent);
    if (isEditing) {
  ...
  const handleFixMissingMeshReferences = () => {
    if (onFixMissingMeshRefs) {
      onFixMissingMeshRefs();
      return;
    }
    const currentContent = isEditing ? editedVizUrdf : vizUrdf;
    const result = fixMissingMeshReferences(currentContent, meshFiles, {
            ...
            <div className="ml-auto">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="h-5 px-2 text-[10px] flex items-center gap-1 text-muted-foreground hover:text-foreground hover:bg-muted/20 rounded-sm transition-colors">
                    Tools
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem
                    onClick={handleFixMissingMeshReferences}
