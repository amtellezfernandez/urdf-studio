...
export const DEFAULT_IK_SOLVER_CHAIN: IkSolverId[] = (() => {
  ...
  const withLocal = mergeSolverChains(base, localOverrides);
  return remoteIkEnabled ? withLocal : mergeSolverChains(["ik-js"], withLocal);
})();
...
export const buildIkStrategies = (
  ...
  if (orientationMode === "ignore") {
    attempts.push(true);
  } else if (orientationMode === "position_first") {
...
const fetchServerSolverChain = async (apiBaseUrl: string): Promise<IkSolverId[]> => {
  const response = await guardedFetch(`${apiBaseUrl}/ik/solvers`, undefined, {
    requiredBackends: FEATURE_GATES.ikRemoteSolve.requiredBackends,
    context: "IK solver registry",
  });
  if (!response.ok) {
    return [];
  }
...
const requestSolverChain = (apiBaseUrl: string) => {
  if (!remoteIkEnabled) {
    solverChainCache.set(apiBaseUrl, DEFAULT_IK_SOLVER_CHAIN);
    return Promise.resolve(DEFAULT_IK_SOLVER_CHAIN);
  }
  ...
  const promise = (async () => {
    ...
    try {
      ...
      if (serverChain.length > 0) {
        const withIkJs = mergeSolverChains(["ik-js"], serverChain);
        const localOverrides: IkSolverId[] = isIkfastAvailable() ? ["ikfast-wasm"] : [];
        chain = mergeSolverChains(withIkJs, localOverrides);
      }
    } catch {
