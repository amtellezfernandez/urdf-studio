...
const CAMERA_SECTION_LABEL_CLASS = JOINT_LIST_CLASS_NAMES.cameraSectionLabel;
...
const CameraEditorPanel = ({
  ...
  const applyCameraSensorPose = useCallback(() => {
    ...
    if (!debugReport?.sensor_pose_joint_frame) {
      return;
    }
  ...
  const compactSelectTriggerClass = "h-5 text-[10px] bg-muted/15 border-border/40";
  const compactSelectContentClass = "bg-popover border-border";
  const compactInputClass = "w-full min-w-[48px] text-[10px]";
          ...
          <BlenderPropertyRow label="Res/FOV" labelWidth="w-14" className="items-start">
            <div className="grid grid-cols-3 gap-0.5">
              ...
              <CameraNumberField
                ...
                onValueChange={(val) => {
                  updateCamera(camera.id, {
                    intrinsics: withIntrinsicsFovDeg(camera.intrinsics, val),
                  });
                }}
                step={1}
                min={1}
...
// Persistent world panel (objects only)
