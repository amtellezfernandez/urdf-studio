...
type HealthActionPanelProps = {
  ...
  onGeneratePhysics?: (densityPresetId: InertialDensityPresetId, repairMode: InertialRepairMode) => void;
...
const buildOverviewLabelValueRows = ({
  statusLabel,
  physicsIssueSummary,
  frameIssueSummary,
  physicsAuditSummary,
...
}): Array<{ label: string; value: string; emphasis?: "result" }> => {
  const rows: Array<{ label: string; value: string; emphasis?: "result" }> = [];
  if (statusLabel) {
...
type ExcludedLinkEntry = NonNullable<HealthActionPanelProps["physicsPlausibilitySummary"]>["excludedLinks"][number];

type ExcludedLinkBucketKey =
  | "voxel-ready"
  | "near-miss"
  | "ghost-geometry"
  | "proxy-review"
...
const buildExcludedLinkBucketKey = (entry: ExcludedLinkEntry): ExcludedLinkBucketKey => {
...
const buildExclusionReasonSummary = (
...
const buildExcludedLinkGroups = (excludedLinks: ExcludedLinkEntry[]): ExcludedLinkGroup[] => {
...
const buildSanitizationSummary = (
  excludedLinks: ExcludedLinkEntry[]
): {
