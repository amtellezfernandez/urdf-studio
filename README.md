...
Openable targets in this clean release are Genesis, MuJoCo, PyBullet, SAPIEN, and Blender. SAPIEN uses a headless physics-only opener in this branch. MJX, Isaac Sim, Isaac Lab, Isaac Gym, and CoppeliaSim/PyRep are listed as planned transfer targets with runtime status detection so their adapters can be filled in without changing the public target contract.
