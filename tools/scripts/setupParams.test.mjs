...
test('Genesis workspace adapter runtime is pinned separately from portable backend setup', () => {
  ...
  assert.equal(GENESIS_TORCH_PACKAGE, `torch>=${GENESIS_TORCH_MIN_VERSION}`);
  assert.deepEqual(GENESIS_PYTHON_DEPENDENCIES, [GENESIS_TORCH_PACKAGE, GENESIS_WORLD_PACKAGE, GENESIS_RENDER_PACKAGE]);
  assert.match(GENESIS_TORCH_PACKAGE, /^torch>=\d+\.\d+$/);
  ...
  assert.match(GENESIS_VERIFY_IMPORT_SCRIPT, /assert torch_version >= min_torch/);
});
...
test('simulator container setup env names live in setup params', () => {
  assert.equal(LOCAL_SIMULATORS_SETUP.installFlag, '--with-simulators');
  assert.equal(LOCAL_SIMULATORS_SETUP.installCommand, 'npm run setup:simulators');
  assert.equal(LOCAL_SIMULATORS_INSTALL_FLAG, LOCAL_SIMULATORS_SETUP.installFlag);
  assert.equal(LOCAL_SIMULATORS_INSTALL_COMMAND, LOCAL_SIMULATORS_SETUP.installCommand);
  assert.equal(SIMULATOR_CONTAINER_SETUP.installFlag, '--with-simulator-containers');
  assert.equal(SIMULATOR_CONTAINER_SETUP.installCommand, 'npm run setup:simulator-containers');
  ...
  assert.equal(SIMULATOR_CONTAINER_INSTALL_FLAG, SIMULATOR_CONTAINER_SETUP.installFlag);
  assert.equal(SIMULATOR_CONTAINER_INSTALL_COMMAND, SIMULATOR_CONTAINER_SETUP.installCommand);
