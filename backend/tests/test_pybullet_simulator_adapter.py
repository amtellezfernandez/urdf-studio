...
from backend.scripts.pybullet_world_open import _primitive_shape
...
def test_simulator_launch_normalizes_expanded_xacro_paths_to_urdf() -> None:
    assert (
        launch_package._normalize_resolved_urdf_asset_path("pkg/urdf/robot.urdf.xacro")
        == "pkg/urdf/robot.urdf"
    )
    assert (
        launch_package._normalize_resolved_urdf_asset_path("pkg/urdf/robot.xacro")
        == "pkg/urdf/robot.urdf"
    )
    assert (
        launch_package._normalize_resolved_urdf_asset_path("pkg/urdf/robot.urdf")
        == "pkg/urdf/robot.urdf"
    )
...
def test_prepare_pybullet_simulator_launch_bundles_uploaded_assets_and_package_roots(
    monkeypatch,
    tmp_path: Path,
) -> None:
    urdf_xml = """
...
""".strip()
    ...
    monkeypatch.setattr(
        ...
        replace(pybullet_adapter.PYBULLET_LAUNCH_PARAMS, launch_root=tmp_path),
    )
    ...
    prepared = pybullet_adapter.prepare_pybullet_launch(request)
...
def test_prepare_pybullet_launch_stages_expanded_xacro_as_urdf(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    prepared = pybullet_adapter.prepare_pybullet_launch(request)
    staged_source_path = (
        prepared.launch_dir / "source" / "demo_description" / "urdf" / "robot.urdf"
    )
...
def test_prepare_pybullet_launch_inlines_named_visual_material_colors(
    monkeypatch,
    tmp_path: Path,
) -> None:
    urdf_xml = """
...
def test_prepare_pybullet_launch_adds_fallback_visual_material_colors(
    monkeypatch,
    tmp_path: Path,
) -> None:
    urdf_xml = """
...
def test_launch_pybullet_world_reports_direct_urdf_transfer(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(pybullet_adapter, "prepare_pybullet_launch", lambda request: prepared)
    monkeypatch.setattr(world_process.subprocess, "Popen", lambda *args, **kwargs: _FakeProcess())
    monkeypatch.setattr(world_process, "wait_for_launch_readiness", lambda *args, **kwargs: None)

    response = pybullet_adapter.launch_pybullet_world(
