...
def _has_positive_diagonal_inertia(inertial: ET.Element) -> bool:
    ...
    return len(values) == 3 and all(
        value > MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal for value in values
    )
...
def _has_positive_full_inertia(inertial: ET.Element) -> bool:
    ...
    try:
        ...
        return False
    if ixx <= MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal:
        return False
    determinant_2x2 = (ixx * iyy) - (ixy * ixy)
    if determinant_2x2 <= MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal:
        return False
    determinant_3x3 = (
        ixx * ((iyy * izz) - (iyz * iyz))
        - ixy * ((ixy * izz) - (ixz * iyz))
    ...
    return determinant_3x3 > MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal
...
def _regularize_full_inertia(
    ...
    base_scale = max(abs(value) for value in values) or MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal

    for attempt in range(MUJOCO_LAUNCH_REPAIR_PARAMS.inertia_shift_attempts):
        diagonal_shift = (10**attempt) * base_scale * 1e-9
        candidate = (
            max(ixx, MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal) + diagonal_shift,
            max(iyy, MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal) + diagonal_shift,
            max(izz, MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal) + diagonal_shift,
            ixy,
            ixz,
            iyz,
        )
...
def apply_mjcf_launch_repairs(mjcf_content: str) -> tuple[str, tuple[str, ...]]:
    ...
    for body in root.findall(".//body"):
        ...
        if (
            mass is not None
            and mass > MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertial_mass
        ...
        if _body_needs_only_frame_inertial(body):
            ...
            warnings.append(f"Launch repair removed invalid frame inertial from MJCF body '{body_name}'.")
            continue

        inertial.set(
            "mass",
            f"{max(mass or 0.0, MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertial_mass):.12g}",
        )
        ...
        regularized_full_inertia = (
            ...
            if full_inertia is not None and (mass or 0.0) > MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertial_mass
            else None
        )
        if regularized_full_inertia is not None:
            ...
            inertial.set(
                "diaginertia",
                f"{MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal:.12g} "
                f"{MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal:.12g} "
                f"{MUJOCO_LAUNCH_REPAIR_PARAMS.min_inertia_diagonal:.12g}"
            )
        warnings.append(f"Launch repair regularized invalid inertial on MJCF body '{body_name}'.")
...
def launch_mujoco_world(
    ...
    *,
    ...
    prepared = prepare_mujoco_launch(
        request,
    ...
    return launch_prepared_world_process(
        ...
        launch_params=MUJOCO_LAUNCH_PARAMS,
        error=_mujoco_error,
